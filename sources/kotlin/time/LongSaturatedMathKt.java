...
public final class LongSaturatedMathKt {
    ...
    public static final long m5058saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM4975toLongimpl = Duration.m4975toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m5057checkInfiniteSumDefinedPjuGub4(j, j2, jM4975toLongimpl);
        }
        if (((jM4975toLongimpl - 1) | 1) == Long.MAX_VALUE) {
            return m5059saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM4975toLongimpl;
        return ((j ^ j3) & (jM4975toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m5057checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m4961isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m5059saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM4932divUwyO8pc = Duration.m4932divUwyO8pc(j2, 2);
        long jM4975toLongimpl = Duration.m4975toLongimpl(jM4932divUwyO8pc, durationUnit);
        return (1 | (jM4975toLongimpl - 1)) == Long.MAX_VALUE ? jM4975toLongimpl : m5058saturatingAddNuflL3o(m5058saturatingAddNuflL3o(j, durationUnit, jM4932divUwyO8pc), durationUnit, Duration.m4964minusLRDsOJo(j2, jM4932divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m5031getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m5030getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m4982unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m4965plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m4982unaryMinusUwyO8pc(infinityOfSign(j3));
        }
